...
> Answer: Client-side routing is faster (after the initial load time) because the page doesn't have to make HTTP requests and load the new page each time. Smoother transitions. Works offline (after the initial load-in). 
